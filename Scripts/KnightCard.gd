...
const BUFF = preload("res://Scenes/KnightBuff.tscn")
...
func _on_AttackButton_pressed():
	if Deck.player_turn == true:
		get_parent().get_parent().get_parent().get_node("BossPos").get_node("Boss").health -= 4
		$Blood.play()
		...
		get_parent().get_parent().get_parent().get_node("ScrollContainer/HBoxContainer").add_child(buff)
		$DiscardSound.play()
